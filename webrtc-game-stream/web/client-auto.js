...
// Session manager for auto-session creation
const sessionManagerUrl = urlParams.get('session_manager') || window.location.hostname;

console.log(`[CLIENT] Base room: ${baseRoom}`);
console.log(`[CLIENT] Session manager: ${sessionManagerUrl}`);
...
async function connectToSessionManager() {
  const smUrl = `ws://${sessionManagerUrl}:8080`;
  setStatus(`Requesting session from ${smUrl}...`);

  sessionWs = new WebSocket(smUrl);

  sessionWs.onopen = () => {
    console.log("[CLIENT] Connected to session manager");
    // Request a session
    ...
    setStatus("Waiting for session assignment...");
  };

  sessionWs.onerror = (err) => {
    console.error("[CLIENT] Session manager error:", err);
    setStatus("❌ Failed to connect to session manager");
  };

  sessionWs.onclose = () => {
    console.log("[CLIENT] Session manager disconnected");
  };

  sessionWs.onmessage = async (msg) => {
    const data = JSON.parse(msg.data);
    console.log("[CLIENT] Session manager message:", data);

    if (data.type === "session_assigned") {
      assignedRoom = data.room;
      sessionId = data.session_id;
      console.log(`[CLIENT] ✅ Assigned to room: ${assignedRoom} (session: ${sessionId})`);
      setStatus(`Session assigned: ${sessionId}`);
...
async function connectToSignalingServer() {
  ...
  const wsUrl = `ws://${sessionManagerUrl}:9000`;
